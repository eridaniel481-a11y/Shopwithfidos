...
exports.handler = async (event) => {
  ...
  // Use the reference number generated once at submit time on the order
  // form (steering-lock/index.html), so it matches what the customer sees
  // on the thank-you page. Only falls back to generating a new one if it's
  // somehow missing, which would only happen on a malformed submission.
  const ref = data.order_ref || 'SWF-' + Date.now().toString().slice(-6);
